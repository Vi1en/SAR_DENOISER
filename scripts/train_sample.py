...
def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Use unified ImprovedTrainer pipeline (trainers.pipeline.run_training) → '
            'checkpoints_improved/. Default keeps DenoiserTrainer / UnrolledTrainer modes.'
        ),
    )
    parser.add_argument(
        '--config',
        ...
        default=None,
        help=(
            'YAML config for improved pipeline (configs/train/). Runs run_training and exits; '
            'same as --use-improved-pipeline with hyperparameters from file.'
        ),
    )
    ...
    if args.config:
        ...
        cfg = training_config_from_yaml_path(cfg_path)
        if not os.path.exists(cfg.data_dir):
            print(f"❌ Processed SAMPLE dataset not found at {cfg.data_dir}")
            print("Please run the dataset downloader first:")
            print("python data/sample_dataset_downloader.py")
            return
        print(f"📄 Training config: {cfg_path.resolve()}")
        run_training(cfg)
        return
    ...
    if args.use_improved_pipeline:
        ...
        dev_str = str(device)
        noise_cond = args.model_type.lower() == "dncnn"
        cfg = TrainingConfig(
    ...
    print("\nTo evaluate: python scripts/evaluate_sample.py")
    print(f"To run demo: streamlit run demo/streamlit_app.py")
